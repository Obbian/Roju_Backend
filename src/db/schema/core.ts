...
export const users = pgTable(
  'users',
  {
    ...
    // Required for She-Bike/She-Share gender-based matching.
    ...
    // CONSUMER_APP / EXECUTIVE_APP / ADMIN_WEB — where the account was created, set once.
...
export const drivers = pgTable(
  'drivers',
  {
    // 1:1 with users.id — no separate surrogate key
    userId: uuid('userId')
      .primaryKey()
      .references(() => users.id),
