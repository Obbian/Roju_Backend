...
export const incidents = pgTable(
  'incidents',
  {
    ...
    // Cross-service soft references to roju-identity's users.id — no DB-enforced FK.
    reportedByUserId: uuid('reportedByUserId').notNull(),
    // counterparty the report is about, when applicable
    againstUserId: uuid('againstUserId'),
    ...
    resolvedByUserId: uuid('resolvedByUserId'),
...
export const cancellationPenalties = pgTable(
  ...
  {
    id: uuid('id').primaryKey().defaultRandom(),
    // Cross-service soft reference to roju-identity's users.id — no DB-enforced FK.
    ...
    waivedByUserId: uuid('waivedByUserId'),
