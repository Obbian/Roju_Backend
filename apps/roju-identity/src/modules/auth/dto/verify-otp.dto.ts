...
export class VerifyOtpDto {
  ...
  @ApiProperty({ example: '123456', description: 'Read from the roju-identity console log in dev' })
  @Matches(/^\d{6}$/, { message: 'otp must be a 6-digit code' })
  otp!: string;

  @ApiProperty({ enum: RegisteredVia, example: RegisteredVia.CONSUMER_APP })
  ...
  @ApiPropertyOptional({ example: 'RIDE' })
  @IsOptional()
  @IsString()
  @MinLength(1)
  serviceCode?: string;
